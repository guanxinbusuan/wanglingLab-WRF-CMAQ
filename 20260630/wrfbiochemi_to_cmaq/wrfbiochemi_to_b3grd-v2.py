...
# 土壤 NO 基准值（g N/km²/h）
# 来源：基于 USGS 24 类土地利用的 BEIS3 经验值。
# 注意：BEIS3.61 中每类的土壤 NO 排放率来自 BELD 数据库的实测汇总，
#       此处使用简化版查表值；正式应用建议与本地观测校准。
SOIL_NO_BASE = {
...
# ===== 物种分组：各物种从哪个 WRF 排放因子推导 =====
# ISOP 衍生物种（从 MSEBIO_ISOP 推导）
ISOP_DERIVED = [
    'AVG_ISOP', 'AVG_MBO', 'AVG_METH',
    'AVG_ETHE', 'AVG_PROPE', 'AVG_ETHO', 'AVG_ACET',
    'AVG_HEXA', 'AVG_HEXE', 'AVG_HEXY', 'AVG_FORM',
    'AVG_ACTAL', 'AVG_BUTE', 'AVG_ETHA', 'AVG_FORAC', 'AVG_ACTAC', 'AVG_BUTO',
    # 'AVG_COS'  ← AVG_CO 已移至 OVOC_DERIVED（从 ef_ovoc 推导，CO 来源于 OVOC 氧化）。
    #               原 ISOP 组中的 AVG_COS 符号含多余 S 后缀（base name 应为 AVG_CO），
    #               且与 OVOC 组重复计算。此处注释保留、实际移除，AVG_CO 由 OVOC 组正确提供。
    'AVG_ORVOC', 'AVG_SESQT',
]
# 单萜烯物种（从 MSEBIO_MONO 推导，C10H16 骨架，碳质量 ~120.10 g C/mol）
MONO_DERIVED = [
    'AVG_APIN', 'AVG_BPIN', 'AVG_D3CAR', 'AVG_DLIM',
    'AVG_CAMPH', 'AVG_MYRC', 'AVG_ATERP', 'AVG_BPHE',
    'AVG_SABI', 'AVG_PCYM', 'AVG_OCIM', 'AVG_ATHU',
    'AVG_TRPO', 'AVG_GTERP',
]
# OVOC 衍生物种（从 MSEBIO_OVOC 推导）
OVOC_DERIVED = [
    'AVG_OVOC', 'AVG_CO',
]
...
def calculate_emissions(ef_isop, ef_mono, ef_ovoc, soil_no_base, lai, grid_area=None, season="summer"):
    """计算BEIS标准排放（单位：gramsC/hour）
       参数 grid_area 保留仅为向后兼容，函数内未使用
       （B3GRD 需要单位面积排放强度，不乘网格面积）
    """
    results = {}
    ...
    # ===== [原逻辑] 单循环遍历 TEMPLATE_VARS，仅用 ISOP（注释保留）=====
    # for var_name in TEMPLATE_VARS:
    #     if var_name.startswith('LAI_'): continue
    #     if var_name in ['AVG_NOAG_GROW', 'AVG_NOAG_NONGROW', 'AVG_NONONAG']: continue
    ...
    #
    # ===== 新逻辑：按三组分别从 ISOP / MONO / OVOC 推导 =====
    # 注：B3GRD 需要单位面积排放强度（g C/m²-hr），不乘网格面积
    
    # ---- 组 1：ISOP 衍生物种（从 ef_isop 推导）----
    for base_name in ISOP_DERIVED:
        ratio = SPECIES_RATIOS[base_name]
        c_mass = C_MASS.get(base_name, 12.01)
        val = isop_base * ratio * (c_mass / C_MASS['AVG_ISOP'])
        for suffix in ('S', 'W'):
            results[f'{base_name}{suffix}'] = val
    
    # ---- 组 2：单萜烯物种（从 ef_mono 推导）----
    # 归一化：各物种按原有比例拆分，组内总和 = ef_mono 的碳质量
    MONO_RATIO_SUM = sum(SPECIES_RATIOS[n] for n in MONO_DERIVED)
    mono_base = ef_mono * C_MASS['AVG_APIN'] / 1e6  # g C/m²-hr
    for base_name in MONO_DERIVED:
        ratio = SPECIES_RATIOS[base_name]
        c_mass = C_MASS.get(base_name, 120.10)
        val = mono_base * (ratio / MONO_RATIO_SUM) * (c_mass / C_MASS['AVG_APIN'])
        for suffix in ('S', 'W'):
            results[f'{base_name}{suffix}'] = val
    
    # ---- 组 3：OVOC 衍生物种（从 ef_ovoc 推导）----
    OVOC_RATIO_SUM = sum(SPECIES_RATIOS[n] for n in OVOC_DERIVED)
    ovoc_base = ef_ovoc * C_MASS['AVG_OVOC'] / 1e6  # g C/m²-hr
    for base_name in OVOC_DERIVED:
        ratio = SPECIES_RATIOS[base_name]
        c_mass = C_MASS.get(base_name, 48.0)
        val = ovoc_base * (ratio / OVOC_RATIO_SUM) * (c_mass / C_MASS['AVG_OVOC'])
        for suffix in ('S', 'W'):
            results[f'{base_name}{suffix}'] = val
    
    # 添加 LAI 变量
    # ⚠️ 近似处理说明：
    # WRF-Chem wrfbiochemi 仅提供网格平均 MLAI（12 个月）。
    # BEIS3 标准中 ISOPS/MBOS/METHS 应有物种特定的 LAI（按 PFT 区分），
    # 但由于缺乏多物种 LAI 数据，此处统一使用同一网格平均 LAI。
    ...
    # 修正：不乘 grid_area（B3GRD 需要单位面积排放强度）
    # [原] 缺失 km²→m² 单位换算：soil_no_base(g N/km²/h) / 1e6 = (g N/m²-hr)
    #     与 VOC 计算的 ef_isop * C_MASS / 1e6 保持一致的 km²→m² 转换
    # results['AVG_NOAG_GROW'] = soil_no_base * no_factor * 0.005
    results['AVG_NOAG_GROW'] = soil_no_base * no_factor * 0.005 / 1e6
    # results['AVG_NOAG_NONGROW'] = soil_no_base * no_factor * 0.002
    results['AVG_NOAG_NONGROW'] = soil_no_base * no_factor * 0.002 / 1e6
    # results['AVG_NONONAG'] = soil_no_base * no_factor * 0.003
    results['AVG_NONONAG'] = soil_no_base * no_factor * 0.003 / 1e6
...
def process_domain(domain, summer_month, winter_month):
    ...
    # 计算排放
    grid_area_km2 = (grid['xcell'] * grid['ycell']) / 1_000_000.0  # 保留计算以备诊断输出，calculate_emissions 内部未使用此值
